...
import com.socialnet.dto.response.VKCitiesRs;
import com.socialnet.dto.response.VKCountriesRs;
import com.socialnet.repository.CitiesRepository;
import com.socialnet.repository.CountriesRepository;
...
public class GeolocationsService {
    ...
    public CommonRs<List<GeolocationRs>> getCitiesApi(String countryName, String starts) {
        ...
        var citiesApi =
                Optional.ofNullable(response).map(VKCitiesRs::getResponse).map(VKCitiesRs.VKCitiesResponse::getItems)
    ...
    public void getCountriesFromVkAndSave() {
        ...
        for (int i = 0; i < isoCodes.size(); i++) {
            ...
            VKCountriesRs.VKCountry ruCountry = responseRu.getResponse().getItems().get(i);
            VKCountriesRs.VKCountry enCountry = responseEn.getResponse().getItems().get(i);
